...
MVC.Test.Controller = MVC.Test.Functional.extend({
	...
	helpers : function(){
		...
		for(var action_name in actions){
			...
			var helper_name = cleaned_name.replace(/(\w*)/g, function(m,part){ return jQuery.String.capitalize(part)}).replace(/ /g, '');
			if(helpers[jQuery.String.capitalize(event_type)])
				helpers[helper_name] = helpers[jQuery.String.capitalize(event_type)].curry(actions[action_name].css_selector);
